...
import LavenderFiltersPanel from './LavenderFilters';
import LavenderSidebar from './LavenderSidebar';
import LavenderPopup from './LavenderPopup';

// ---------------------------------------------------------------------------
// Map pin icons
// ---------------------------------------------------------------------------
function createPinIcon(locationType: string, selected = false) {
  const color =
    LOCATION_TYPE_COLORS[locationType as LavenderLocationType] ?? '#7B5EA7';
...
export default function LavenderMap({ fields }: Props) {
  const [filters, setFilters] = useState<LavenderFilters>(DEFAULT_FILTERS);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [flyTarget, setFlyTarget] = useState<[number, number] | null>(null);
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const filtered = useMemo(() => filterFields(fields, filters), [fields, filters]);

  const handleSelectField = useCallback((field: LavenderField) => {
    setSelectedId(field.id);
    setFlyTarget([field.lat, field.lng]);
    setSidebarOpen(false);
  }, []);

  const handleFiltersChange = useCallback((next: LavenderFilters) => {
    setFilters(next);
    setSelectedId(null);
    setFlyTarget(null);
  }, []);
    ...
    <div className="relative flex flex-col lg:flex-row h-[calc(100dvh-56px)] overflow-hidden">
      ...
      <div className="flex-1 relative h-full">
        ...
        >
          ...
          {fields.map((field) => {
            const isVisible = filtered.some((f) => f.id === field.id);
        ...
        >
          {sidebarOpen ? (
            ...
            <>
              ☰ Lavender Fields
              {activeFilterCount > 0 && (
                <span className="bg-[#7B5EA7] text-white text-xs rounded-full w-5 h-5 flex items-center justify-center leading-none">
                  {activeFilterCount}
                </span>
              )}
            </>
        ...
        {LAVENDER_LOCATION_TYPES.map((type) => (
          <span key={type} className="flex items-center gap-1.5 text-xs font-body text-charcoal/70">
            <span
              ...
              style={{ backgroundColor: LOCATION_TYPE_COLORS[type as LavenderLocationType] }}
            />
